...
Видимый `.web` сохраняет исполнителя даже при пустом отдельном поле JavaScript:
inline-обработчики и редакторы принадлежат HTML. Его роль `.liveProgram`
отделена от коротких `.visible`/`.background` заданий снимка. Общий предел —
шесть WK; при обычных лимитах три пассивные программы оставляют один пассивный
слот подготовке соседей и два прежних слота текущему листу/вводу.
`runtimeOwners` когорты выбирает этот ограниченный набор независимо от native
geometry slots. Остальные видимые программы получают настоящий растр и явное
локальное состояние «Программа приостановлена» с действием «Запустить».
Действие запуска получает допуск ввода; готовая HTML-кнопка получает уже
следующее настоящее нажатие, без воспроизведения прежних координат.
Текущий лист ограничивает пассивные программы тем же бюджетом.
...
покрытия, затем снимаются необязательные живые элементы.
