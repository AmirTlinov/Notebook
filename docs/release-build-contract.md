...
селекторе не скрывается за успехом соседнего теста. Неизвестная область правки
не допускается в выбранный выпуск.
...
`./verify.sh --only --profile … --test …` сохраняет `selectionMode: explicit-only`.
Сборщик читает точный исполненный набор и неизменные входы, не приписывает ему
остальные профили или общий UI. Пустой набор, неизвестный владелец, отсутствующий
сценарий UI-фикстуры, опечатка селектора, пропуск и ошибка по-прежнему дают отказ.
Выбор достаточного контракта остаётся явным инженерным решением; `--only` не
заменяет полную физическую приёмку приложения.

## Явно выбранная область

Выбор `--only --profile/--test` является явным решением о достаточной области,
в том числе для общего файла модели с несколькими владельцами. Квитанция
сохраняет названные проверки и не классифицированные автоматически файлы;
сборщик принимает её как выбранную, не полную. Без явного выбора неизвестные
исходники запрещают выпуск. Общая UI-фикстура по-прежнему требует конкретного
жестового сценария; успешный широкий профиль не заменяет его. Неизменность
исходников, результаты команд, исполнение названных тестов и подписи проверяются
в обоих случаях.
