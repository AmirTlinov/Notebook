...
Simulator-проверка проводит обратимый путь `лист -> доска -> лист` одной парой
щипков.
...
Щипок привязан к его экранному центру: мировая точка под пальцами остаётся под
ними. Вблизи обложки гладкое поле притяжения переводит масштаб в прогресс
открытия, а после отпускания пружина выбирает ближайшее устойчивое состояние
`board`, `cover` или `page`. Сам открытый лист не зумируется: уменьшение уже
означает закрытие тетради.
