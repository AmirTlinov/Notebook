...
десятипроцентного отхода и может снова вернуть его.
После отпускания пружина выбирает одно устойчивое состояние `board`, `cover` или
`page`. Закрытая обложка выпускает на доску после уменьшения до `0,94` её
фокусного размера либо при продолжающемся быстром движении внутрь. Нижняя
...
радиального движения сохраняется отдельно от этой новой базы, поэтому короткий
щипок не исчезает между распознаванием и первым новым измерением масштаба.
