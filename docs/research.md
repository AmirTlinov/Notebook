...
Simulator-проверка закрывает лист, оставляет небольшой щипок обычным масштабом
доски, затем открывает целый лист большим щипком. Отдельный путь открывает ту же
тетрадь двойным касанием.
...
`NotebookOpeningIntent` передаёт ему фокус после отношения расстояний `1,5` либо
после быстрого разведения не меньше `1,28`, подтверждённого полным 80-мс окном
измерения. Поэтому короткий щипок приближает саму доску и сохраняет выбранный
масштаб. В момент подтверждения прогресс открытия равен нулю; следующий участок
того же движения раскрывает обложку по логарифму отношения расстояний. Обратное
движение может снять подтверждение. После отпускания пружина выбирает одно
устойчивое состояние `board`, `cover` или `page`. Сам открытый лист не имеет
отдельного зума: уменьшение означает закрытие тетради.
...
ввод лишь выключается. При подтверждении другой тетради этот единственный
владелец переносится на её выбранный лист. Открытие меняет видимость и
доступность этого Metal-владельца, а `PKDrawing` преобразуется в геометрию только
при смене данных листа.
