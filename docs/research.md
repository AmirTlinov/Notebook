...
| Форма физической тетради | [SwiftUI `RoundedRectangle`](https://developer.apple.com/documentation/swiftui/roundedrectangle) | Стиль `continuous` создаёт единую плавную кривую угла | Лист, лицевая и обратная стороны обложки используют радиус в один физический сантиметр и одну continuous-форму |
...
Эллиптическое поле с ореолом `1,12` над обложкой даёт только кандидата.
`NotebookOpeningIntent` принимает радиальное движение после отношения расстояний
`1,1` либо быстрое движение после `1,05`, подтверждённое полным 80-мс окном.
Этого свидетельства достаточно лишь возле тетради: фокус передаётся, когда её
масштаб достиг канонического `coverScale`. Поэтому даже сильный щипок вдали
приближает доску, а раскрытие начинается с нуля на действительно близкой
обложке. Следующий участок той же камеры открывает её до `pageScale`; обратное
движение снимает фокус после десятипроцентного отхода и может снова вернуть его.
...
намерение открытия получают одну и ту же честную базу.
...
обложки обрезаются одной continuous-формой радиусом в физический сантиметр.
