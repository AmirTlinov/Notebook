...
`notebook_present` — явная просьба показать материал, не побочный эффект чтения,
вставки, ответа в чате или перехода по ссылке на код. Агент получает текущий вид
вызовом без аргументов, затем передаёт его `view`, новый `presentation_id` и
короткую последовательность `steps`. Поля `camera` задают центр и масштаб;
...
который агент может сформировать скриптом и отправить одним вызовом; отдельного
движка произвольного JavaScript в приложении не создаётся. Временное SVG может
