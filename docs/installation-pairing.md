...
Private zone push-подписка сообщает об изменениях директории через AppDelegate.
В private database остаётся ровно один CKSyncEngine — владелец обмена содержанием. При ошибке работает ограниченный
...
Поэтому device directory использует CKRecordZoneSubscription и push через
AppDelegate, а не второй engine. Push лишь будит повторное чтение private
account record; сам payload никогда не даёт доверие. При первом запуске чтение
повторяется после установки подписки, чтобы закрыть промежуток между enrollment
и подпиской. iPad background callback ждёт завершения account owner; выключение
обмена содержанием не выключает обнаружение своих устройств.
