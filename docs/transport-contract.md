...
## Общая интеграция: протокол 33 / manifest 15 (в разработке)

Единый декодер объединяет пакеты программ и полный TeX GUI-240 (28/11)
с нативными инструментами, текстом и круглым ластиком основной ветки (32/14).
Ни одна прежняя ветка не объявляется совместимой с объединённым содержанием:
сопряжение требует обновления обоих приложений. Manifest 4–14 читаются без
...
В текущих development-исходниках протокол 33 и manifest 15 объединяют полный TeX,
пакетные программы и нативные инструменты с каноническими размещениями доски 3
...
manifest 4–14 остаются читаемыми без переписывания истории.
...
Bonjour объявляет версию обмена в имени `notebook-v33-<UUID>-<поколение>`; прежний writer
