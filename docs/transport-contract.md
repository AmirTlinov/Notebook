...
В текущих исходниках протокол 22 и manifest 8 передают канонические размещения
доски 3. Версия 22 добавляет сессионное выделение, не меняя manifest содержания.
...
Формат локального пространства, ключи и подтверждения пары не меняются;
...
Bonjour объявляет версию обмена в имени `notebook-v22-<UUID>-<поколение>`; прежний writer
