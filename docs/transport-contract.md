...
сокета не считается завершением SQL-команды. Возвращаемая длительность
заканчивается в уведомлении сервера о завершении, до ожидания вызывающим
кодом собственного исполнителя. Проверка неполного кадра сохраняет строгий
предел меньше секунды именно для этой границы; отдельно записывается полное
ожидание вызвавшего теста. Долгая синхронная работа соседних тестов не выдаётся
за задержку закрытия сокета.
...
принятой команды после отключения клиента. Некорректный peer отправляет raw
заголовок, но принимает ответ тем же `SocketIO`; отдельного запасного reader нет.
Профиль из 13 тестов и нагрузочный профиль 398 Core прошли. Последний исключал
четыре минутные проверки 100 000 владельцев и **не является полным маршрутом**;
итоговый неизменный `verify.sh` учитывается отдельно в [verification.md](verification.md).
