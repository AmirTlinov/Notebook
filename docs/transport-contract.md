...
## Единая печатная интеграция: протокол 28 / manifest 11 (в разработке)

Полный исходник `.tex` и canonical PDF требуют декодера нового вида блока.
Граница 28/11 включает также пакетные программы GUI-242 и eraser версии 116;
она не переиспользует опубликованную development-границу 27/10.
Сопряжение с другой wire-version явно требует совместного обновления. История
manifest 4–10 читается без перезаписи; `.tex` в manifest до 11 отвергается.
Установленная физическая пара 116 (26/9) этим изменением не обновляется.
...
В текущих development-исходниках протокол 28 и manifest 11 добавляют полный TeX
и пакетные программы к выпущенным в 26/9 каноническим размещениям доски 3 и цельному стиранию
SVG/HTML в существующих действиях чернил. Старый
получатель не должен молча интерпретировать `wholeElement` как частичную маску;
manifest 4–10 остаются читаемыми без переписывания истории.
...
Bonjour объявляет версию обмена в имени `notebook-v28-<UUID>-<поколение>`; прежний writer
