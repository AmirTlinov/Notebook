...
Проверочный некорректный peer вручную отправляет ошибочный четырёхбайтный
заголовок, но получает ответ тем же `SocketIO`. Его прежние отдельные `take()`
и трёхсекундный timeout удалены. Детерминированный RED показал четыре отсутствующих
настройки raw-сокета; GREEN **13/13** проверяет настройки, точные байты тела,
оборванные заголовок и тело, отказ по длине при ещё открытом peer и отсутствие
вызова владельца данных. Источник из 126 файлов не менялся:
`.build/ipc-malformed-frame-20260909/{policy-red-core,green-core}/`.
Эта проверка не приписывает продукту недоказанную гонку: старый тест потерял errno
своей ошибки Cocoa256. Последующий полный маршрут остаётся обязательным.
