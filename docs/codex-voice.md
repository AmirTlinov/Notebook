...
Удержание микрофона включает диктовку по обращению с отправкой после паузы;
это отдельный способ начать ту же диктовку, не голосовой звонок.
...
не меняются. Для явно включённого обращения действует следующий контракт.
...
Удержание микрофона в чате или компаньоне открывает «Включать по GPT».
После явного включения `NotebookDictationController` принимает обращение через
...
последовательно выполняет актор вне UI; очередь ограничена шестнадцатью порциями.
Потерянный кадр или переполнение останавливают захват, а не склеивают чужие фразы.

Когда просьба закончилась, 1,4 секунды тишины завершают запись. Одно имя ждёт
...
Значок уха обозначает ожидание, волна — приём речи, строка обработки — передачу
и распознавание. Нажатие на ухо выключает микрофон и ожидание. Смена задачи,
компьютера, потеря связи, уход в фон и начало звонка прекращают ожидание;
уже начатая запись закреплена за исходной задачей. Кнопка разговора по-прежнему
сразу начинает звонок, без имени GPT. Имя и обычный текст просьбы не являются
ответом на отдельное ожидающее разрешение. После перезапуска микрофон выключен.
