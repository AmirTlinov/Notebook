...
Внутри чата находится сворачиваемое дерево: каждая раскрытая папка возвращает
...
Рукопись и привязка агентских объяснений относятся к следующему срезу GUI-185,
а проверка и запуск процесса — к GUI-187; наличие текстового редактора их не
подменяет.
...
приоритет. Протокол пары — 4; прежний peer не получает неизвестную команду.
...
`NSFileCoordinator` согласует замену с участвующими редакторами. Дескриптор,
...
папки и восстановление без повторной записи. `NotebookFileControllerTests`
