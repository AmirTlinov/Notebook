...
На iPad исходящий ввод получает UUID и сохраняется до отправки. Повтор одного
UUID с другим содержанием — ошибка. Sidecar сохраняет тот же ввод и переводит
...
На изолированном дереве от `6527a85` с изменениями чата прошли 22 iPad/Simulator
и 7 Mac runtime-тестов без пропусков и предупреждений исполнения. Проверены
реальный TLS-круг сообщения/квитанции, потерянный ответ и порядок нескольких
offline-сообщений, восстановление sidecar, ожидание занятой задачи, неизменяемое
указание, три формы TeX и сворачивание/поворот панели без сдвига бумаги.
Дополнительно прошли TypeScript check и 40 MCP-тестов, включая запуск поставляемого
пакета без внешнего `node_modules`; Core-доставка ранее прошла 9 тестов в `035ac6d`.
Перед фиксацией все 496 файлов изолированного дерева сверены с индексом среза.

Повторная native-проверка 13:39 UTC использовала настоящую задачу Codex
`01a08b8b-80a4-72a2-a57f-a6a33366686c`: ответ, явный отказ разрешению, остановка
точного хода и чтение истории. После проверки задача архивирована. Квитанция
моста хранит именно исходники этого запуска; обновлённый помощник регистрации
проверен последующей Mac-сборкой, но регистрация живого Notebook не менялась.
Полный `./verify.sh` на итоговом срезе и физическая приёмка ещё не выполнены.

Удаление исполнительных API Core затем проверено отдельно: 88 профильных Core
и 12 проверок переноса, TypeScript check и 40 MCP-тестов прошли на изолированных
исходниках. Исторические остановки и исходники в проверочных архивах теперь
задаются типизированными значениями, а не запуском удалённого исполнителя.
