...
Правая область панели ограничена показанными рамками инструментов и подключения:
она не закрывает навигацию страниц и создание предметов. Закреплённая карточка
избегает этой же рамки; её отдельный контейнер доступности сохраняет обе кнопки.
...
содержания, не текст ответа Codex. После очистки редактора ещё не принятый текст
виден из той же устойчивой очереди: до четырёх последних карточек, общий счётчик
и явное наличие более ранних сообщений. При native client ID или квитанции
карточка исчезает; это не вторая редактируемая история.
...
Общий неизменный срез `b480dac` дошёл до iPad и завершился отказом:
