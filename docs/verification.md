...
## Установленный рабочий срез: 0.3.27 (30)

Commit `0aa0194` установлен на нынешние Mac и физический iPad без замены
контейнеров, идентичностей или доверия. Подписанная пара получена через
`Applications/build-verified-pair.sh` из `.build/milestone-027-check`;
сборка и подписи — `.build/milestone-027-release/build.json`.
SHA-256 исходников:
`c46b3d3123520ed152c67290b496535f91d7cb1b01ef79219bcf72182dd3e12e`.

Объединённый выбранный маршрут прошёл **73 Core, 17 Codex, 2 внешние,
45 MCP, 27 Mac и 31 iPad/UI** проверок; также 40 проверок сборщика и 25 выбора.
Ошибок, пропусков и runtime warnings нет. Это достаточный выбранный маршрут
затронутых владельцев, не полный прогон всего приложения. Настоящие жесты
Simulator проверили Pencil на нативном коде, свою отмену, перепривязку пометки,
сворачивание чата, клавиатуру и поворот без движения бумаги. При перепривязке
исходный текст остался прежним; просмотренный снимок сохранил рисунок рядом
с ним. Simulator принимает тестовое прямое
касание как Pencil только внутри своей заготовки; физический Apple Pencil этим
не удостоверен.

Перед обновлением создана и прочитана обратно независимая копия
`/Users/amir/Documents/Notebook Backups/20260912-0451-before-code-notes`.
SQLite обеих копий прошла integrity_check; сохранены также активации, настройки
и прежнее Mac-приложение. Mac завершён штатно, iPad остановлен без активной работы,
после чего оба приложения обновлены на месте и запущены обычным путём.

Установленный Mac совпал по полной описи с подписанным bundle; физический iPad
показывает Notebook `com.amirtlinov.notebook.preview`, версию 0.3.27 (30);
прежний `com.amirtlinov.notebook` отсутствует. Отдельный XCTest runner не
является другой сборкой Notebook и не владеет его контейнером.
Все **793 файла активации Mac и 519 iPad** остались прежними. Все 1318 записей
Mac вне `runtime/input.json#` сохранили прежние хеши; обновилось только текущее
состояние ввода. Ревизии каталога, доски и чернил не изменились.

Живой MCP именно из установленного bundle объявляет **0.3.27 и 20 инструментов**,
видит соединённый iPad. Камера до и после запуска совпала точно: центр
`(530.6211950340753, 145.427845931444)`, tile `(0,0)`, scale
`0.25673606914481567`, viewport `834×1194`, mode `board`, без focused item.
Снимки до и после установки сохранены; обложки и прежние штрихи на месте.
Подробная квитанция — `.build/milestone-027-install/installed.json`.
...
Обе попытки `.build/physical-milestone/inspect.xcresult` и `inspect-027.xcresult`
закончились `Timed out while enabling automation mode`. Последняя длилась
62,762 секунды, не исполнила ни одного теста и сохранила отказ запуска runner.
...
MacBook-Air-Amir.local ещё не разрешён. Новый Mac нельзя включить копированием
