...
## Подготовленный срез: компактный чат и нативный доступ

12 сентября `.build/chat-corners-checked` проверил неизменные исходники
`33a7a9360888402058fd951ff86a5c0765bda78ff5b266c7c6b8997ae81b21ae`:
**45 Core/Codex, 19 Mac и 26 iPad/UI** проверок, без ошибок, пропусков и runtime
warnings. Область — файлы, разговор, решения Codex и два настоящих жестовых
сценария камеры; это не полный прогон. Просмотрены реальные снимки смонтированного
чата с текстом, компактным запросом и файлами справа. Иконка доступа находится
рядом с «+», а не в отдельной строке. Большая промежуточная карточка не выпускается.
Вызовы имеют контурные иконки своего действия вместо стандартных треугольников.
Настоящие жесты каждого из четырёх углов изменили размер без кнопки растягивания:
противоположная точка, камера, принятые чернила и восстановление окна сохранены.
...
пользовался, настоящие задачи и общие настройки не менялись. Разовое согласие,
срок `session`, постоянный `always` для MCP и точный объём файлового разрешения
проверены на протоколе; постоянное разрешение настоящему инструменту тест не выдавал.
Неизвестная выдача доступа не исполняется повторно и не мешает более позднему
явному выбору. Произвольные формы MCP с дополнительными полями по-прежнему
требуют Codex на Mac; пустая форма согласия больше не показывает один отказ.

Окончательная квитанция выше отделена от исправленных ошибок подготовки:
`chat-access-final` — пропущенная скобка, `chat-compact-final` — неверное чтение
порядка очереди и выбор тестовой задачи до ответа каталога. `chat-access-release`
остановлен после нового требования Амира к виду, не назван успешным.
На момент этой записи новая пара ещё не установлена.

## Установленный рабочий срез: 0.3.28 (31)

Commit `fb37154` установлен на нынешние Mac и физический iPad без замены
контейнеров, идентичностей или доверия. Сборка проверенной пары —
`.build/file-io-owner-pair/build.json`, установка — `.build/file-io-owner-install`.
SHA-256 исходников:
`94df4c2850da990526302aa120dda3cb7e047971a6975070bb9176ce1867d062`.

Срез исправляет обнаруженное на установленной 0.3.27 зависание: чтение папки
ожидало `open` внутри общей очереди Notebook. В 0.3.28 такое ожидание принадлежит
одному отдельному файловому потоку; SQLite продолжает сохранять и отвечать.
Выбранный неизменный маршрут `.build/file-io-owner-final` прошёл **42 Core,
45 MCP, 17 Mac и 13 iPad** проверок без ошибок, пропусков и runtime warnings.
Настоящий `NSFilePresenter` удерживал файл, пока та же очередь сохраняла
черновик; проверены отмена, поздний возврат доступа и холодное восстановление
реального переименования без повтора. Это целевая проверка, не полный прогон.

Перед обновлением создана и прочитана обратно независимая копия
`/Users/amir/Documents/Notebook Backups/20260912-0815-before-file-io-owner`.
SQLite обеих копий прошла integrity_check. При сверке живого Mac изменился только
производный `previews/runtime.json`; базы, содержимое и активация остались
побайтно прежними. Обе копии, их описи, настройки и прежнее Mac-приложение
сохранены. Обычный выход старого Mac не завершился: после проверки отсутствия
активных контактов, запусков и начатых файловых записей остановлен именно
процесс с зависшим чтением. Все 1325 записей до/после этой остановки совпали.

Установленный Mac совпал по полной описи с подписанным bundle. Физический iPad
показывает `com.amirtlinov.notebook.preview`, версию 0.3.28 (31).
Все **793 файла активации Mac и 519 iPad** остались прежними. После обновления
не изменились **1324 записи содержания Mac и 1313 iPad**; обновилось только
`runtime/input.json#`. Совпала вся сохранённая `SessionPresence`, включая
новую выбранную человеком камеру: центр local `(1127.3528591423378,
6395.930287785178)`, tile `(0,-1)`, scale `0.16847757418792092`, viewport
`834×1194`. Прежнее положение из предыдущего выпуска не восстанавливалось.
Снимки физического экрана до/после показывают те же обложки и положение доски.

Живой MCP установленного bundle объявляет **0.3.28 и 20 инструментов** и видит
соединённый iPad. `mac-after.sample.txt` подтверждает настоящее ожидание файлов
у нового отдельного владельца, одновременно MCP и SQLite отвечают. Дополнительный
обычный выход 0.3.28 занял больше 15 секунд, но завершился без сигнала;
после повторного запуска MCP ответил за 620 мс, камера осталась прежней.

**Уточнение файлового доступа, 12 сентября 11:05:** журнал `tccd` подтвердил
явное согласие человека на Documents для `com.amirtlinov.notebook.mac`.
Старое дерево на iPad не повторило неудавшееся чтение и продолжало показывать
ошибку; исправление восстановления находится в подготовленном срезе выше.
До установки и чтения дерева на устройстве файловая приёмка не объявляется
пройденной. Ожидавшее на iPad
создание разговора доставлено с прежним UUID `F4F21136-9CBE-4519-9B16-7D6E946F5F35`,
но Codex вернул тайм-аут: исход удержан как неизвестный, повторного создания
после холодного запуска нет. Сам разговор этим не считается созданным.
...
- Настоящий PTY процесса Mac, ввод/вывод, остановка и перезапуск из чата.
  `.build/project-runs-live.json` подтвердил русский текст и emoji, изменение
