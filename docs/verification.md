...
## Подготовленное обновление: 0.3.30 (33)

12 сентября объединённая целевая проверка `.build/board-terminal-checked`
завершилась успешно: **43 Core/Codex, 46 MCP, 17 Mac и 46 iPad/UI**,
без ошибок, пропусков и runtime warnings. SHA-256 исходников:
`2033c4096efe8d198fda42ec45043e8b20889684db4b9cdd09db8c5037a25ad2`.
Это проверка терминала, чата, композиции, чернил и квитанций, не полный прогон.
Подписанная пара и установка ещё готовятся; текущая установленная версия ниже.

Терминал открывается под разговором и полем сообщения, сначала на половину
высоты; разделитель сохраняет выбранный размер. Явное открытие создаёт оболочку
...
PTY Codex: кириллицу, размер, Ctrl-C, холодное чтение и завершение.
...
`erasure-after-cold-start` просмотрены: линия появляется, стирается, новая модель
читает обе активные записи и показывает стёртые пиксели. Камера не меняется.
Тот же случай до исправления не показывал линию за пять секунд;
...
пределе ожидания 3 секунды. Снимок открытого листа просмотрен.
Точная область 2048×2048 points готовится в 2× без лишней полной маски;
проверены ластик на стыках, дробная проекция и хеш PNG прежнего источника.
[Контракт и границы](board-publication.md).

## Установленный рабочий срез: 0.3.29 (32)

12 сентября на нынешние Mac и физический iPad установлены `1af4f57` и
`7def6ca`: файлы справа от переписки и поля сообщения с одним разделителем,
компактные запросы, нативный выбор доступа одной иконкой рядом с «+», смысловые
иконки действий вместо треугольников и изменение размера за любой из четырёх
углов без отдельной кнопки. Открытие документа и изменение окна не меняют камеру.

Неизменный маршрут `.build/chat-corners-checked/verification.json` прошёл
**45 Core/Codex, 19 Mac и 26 iPad/UI** проверок без ошибок, пропусков и runtime
warnings. Область — файлы, разговор, разрешения и два жестовых сценария камеры,
не полный прогон проекта. В Simulator настоящий жест каждого угла сохранил
противоположную точку окна, бумагу и принятые чернила; сворачивание и холодный
запуск восстановили геометрию. Проверены открытие, прокрутка, редактирование и
закрытие кода без движения бумаги. Просмотрены снимки смонтированного чата
с текстом, компактным запросом, смысловой иконкой и файлами справа.

Подписанная пара — `.build/chat-compact-pair/build.json`; SHA-256 исходников:
`33a7a9360888402058fd951ff86a5c0765bda78ff5b266c7c6b8997ae81b21ae`.
До установки создана и прочитана независимая копия
`/Users/amir/Documents/Notebook Backups/20260912-1155-before-chat-access`.
Согласованный снимок WAL Mac и архив iPad прошли integrity_check; сохранены
описи, активация, настройки и прежнее Mac-приложение. Mac завершён обычным
...
`.build/chat-access-install/installed-readback.json` подтверждает **1325→1325**
записей Mac и **1314→1314** iPad. Изменился только `runtime/input.json#` новой
сессии; всё содержание и SessionPresence совпали. Обе активации побайтно прежние,
SQLite исправны. Все строки окон, очереди, черновиков, сохранений и переименований,
а также настройка геометрии чата iPad сохранились. Текущая выбранная человеком
камера: local `(1237.1597583048638, 453.2549973574587)`, tile `(0,0)`, scale
`0.16847757418792092`, viewport `834×1194`; старое положение не возвращалось.

Установленный Mac сверён с подписанным bundle, физический iPad объявляет
`com.amirtlinov.notebook.preview` версии **0.3.29 (32)**. Живой MCP установленного
помощника объявляет **0.3.29, 20 инструментов**, отвечает `ready` и видит прежний
подключённый iPad. Квитанция — `.build/chat-access-install/installed-mcp.json`.
Снимок `ipad-after.png` в той же папке показывает холодный запуск с прежними
обложками и камерой. Чат на этом снимке свёрнут: это не физическое доказательство
чтения дерева, новых углов или запроса доступа.
