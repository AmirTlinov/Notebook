...
### Установленный результат 113

`03cf876` и `f9b7c99` включены в общий срез GUI-238 как `4d5ec9f` и `1cf56f8`.
Проверенная/установленная пара **0.3.110 (113)** имеет source SHA-256
`a66f36e8f2c8c6aafd730d3c426b85416f514790dd687c0b3e908f1591be01e3`.
Общий selected gate: **15 iPad + 2 Mac PASS**, runtime warnings 0, skips 0.
Это не полный прогон; 4 native camera checks и 3 physical UI camera scenarios
относятся к данной правке. Остальные сценарии проверяют совместный срез GUI-238.

Свидетельства общего неизменного среза:
`/Users/amir/.codex/worktrees/notebook-print-layout/Notebook/.build/gui238-source113-final-v4/`,
сборка `.build/gui238-release113/build.json`, установка `.build/gui238-install113/`
в том же worktree. Установка подтверждена 18 сентября в 20:32 UTC; Mac bundle
и CoreDevice inventory показывают 113, установленный MCP — `connected`, прежнее
пространство `FAAAC405-8EF9-4FB9-9B93-CBD876FA97A4`, cursor 15528.
Данные, контейнеры, ключи и доверие не заменялись.

На физическом iPad выполнены два последовательных pinch-out 0.28 для тетради
и документа: настоящий frame листа остаётся fitted, центр не сдвигается.
Затем pinch-in 1.6 увеличивает лист и сохраняет увеличение после отпускания.
Отдельно проверены перелистывание, явный «Назад» и повторное открытие.
Экспортированные изображения осмотрены лично:
`.build/gui233-paper113/attachments/02D8EEA6-AAE4-4B54-899B-D5224B092C23.png`
(тетрадь после повторного уменьшения),
`04FFDD8B-DC87-4467-B8B4-B4F2082E5A5F.png` (документ),
`F1ED45C1-8AC3-4CC7-963F-5D26B44314D3.png` (реальное увеличение).
Это проверка реального жеста на изолированном содержании физического устройства,
не утверждение о ручной приёмке Амиром его рабочего пространства. Остальные
жалобы GUI-233 на тайлы/движение данным срезом не закрываются.
...
## 19 сентября 2026 — интеграционный срез всех сохранённых веток

По запросу Амира после обнаружения старого редактора в обычном приложении
подготовлена `codex/notebook-integration`. Это временный интеграционный срез,
не новая независимая продуктовая ветка; целевой общей веткой остаётся
`codex/notebook-ipad-reliability`.

В графе среза присутствуют все семь исходных branch heads на 13:03 МСК:
`gui-240-visualizations` (`75a9b39`), `notebook-ipad-reliability` (`b0470f4`),
`notebook-print-layout` (`a1cbfad`), `notebook-mac-workspace` (`552f9ab`),
`notebook-drag-shadow` (`99b70dc`), `notebook-offline-sync` (`b5fed1e`) и
`notebook-recipes` (`f35ac6e`); общий префикс имён — `codex/`.

Конфликт `PenControlsView` разрешён совместно: сохранены embedding в новую
панель документа и настройки повторным нажатием активного инструмента.
Нативный `DocumentSourceWorkspace` остаётся единственным редактором; прежнее
HTML-поле с «Отменить / Сохранить» не возвращено. В настройках обоих targets
сохранены linker flags нового typesetter и версия 117 основной ветки; это
не объявление нового выпуска. Camera/presence сохраняет более новый
`preservingPresence`, а историческая запись физической проверки 113 добавлена
без изменения её области.

Код `f35ac6e` побайтово совпадает с уже перенесённым `ebdfab8` в области навыка
и его тестов. Поэтому история старой ветки присоединена без отката более новых
рецептов и без восстановления четырёх намеренно удалённых шаблонов документов.
Удалён один ранее оставшийся diff3-маркер в исторической записи этого файла.

Проверка интеграционного среза (`.build/branch-integration/`):
- Python verification routes: **80 PASS**; release guards: **67 PASS**.
- JavaScript document input/link route: **7 PASS**, без пропусков.
- Swift parser для объединённых PenControlsView/DrawingResponsivenessTests — PASS;
  XcodeGen разобрал итоговый project.yml, обе версии и linker flags согласованы.
- Исходники до/после этих проверок совпали, SHA-256
  `57adefd99ae6e2b036fd250f7fbd40704eb21c77e98f5663178947bab638771a`.
- Интеграционная разница относительно GUI-240 прошла `git diff --check`.

Это **не сборка приложения, не UI-приёмка и не установка**. Основной каталог
содержит продолжающиеся незакоммиченные правки GUI-259, GUI-240 — собственные
незавершённые изменения. Их файлы, ветки, индексы и запущенные приложения
не переключались; их незакоммиченный код в срез не включён. Физический iPad
и Simulator не затрагивались.

Для завершения перехода на одну рабочую ветку владельцы текущих правок должны
сначала закончить и зафиксировать свои изменения. После их присоединения и
проверки общего результата основная ветка переводится на интегрированный tip;
до этого старый редактор в установленном приложении не считается исправленным.
