...
## Восстановление выбранного компьютера: исходники GUI-189

`.build/computers-final-second` прошёл 25 Core и 15 iPad проверок без ошибок,
пропусков и runtime warnings. После сохранения читателя терминала при отказе
смены повторены компьютерные сценарии и панель в `.build/computers-switch-finish`.
Проверены два адреса одного пути, отдельные черновики и окна, точная маршрутизация
команд, холодный запуск, отказ записи, повтор и сохранение материала после отзыва.
25 проверок выбора также прошли. Первые проходы выявили ошибки тестовых заготовок
(адрес команды, отсутствующее присутствие, testable import); конечные результаты
относятся к исправленным исходникам.

Камера и общая ревизия содержания не меняются при выборе компьютера. Этот срез
не устанавливал приложение и не подключал физический второй Mac. Подготовка
нового устройства к нынешнему пространству и физическая приёмка остаются открытыми;
[контракт](computer-workspace.md).

## Голос: исходники GUI-191, частичный результат

`.build/voice-verified` прошёл 18 Core, 17 Codex, 11 Mac и 10 iPad проверок
без ошибок, пропусков и runtime warnings. `.build/voice-surface` отдельно
прошёл две нативные проверки: локальный WebKit имеет защищённый WebRTC-контекст,
но простое монтирование не создаёт микрофон или соединение; сообщение о
недоступной диктовке не меняет черновик и не отправляет задачу. После защиты
живого звонка от вытеснения историей повторены 14 Core и 17 Codex проверок
в `.build/voice-final-core.log`.

`MacNotebookVoice` сохраняет попытку в прежнем журнале, не повторяет старое
предложение после перезапуска и не выдаёт его чужому устройству. Нативный
контроллер держит голос независимо от сворачивания чата, выключает микрофон
перед завершением и при обрыве. Операции подключения, выключения микрофона,
завершения, перенос разговора в задачу и перебивание на физическом iPad ещё
не приняты. Установленная пара пока 0.3.25 (28).

Отдельная диктовка через авторизацию Codex заблокирована отсутствием
поддержанного интерфейса в текущем App Server. Голосовой разговор не заменяет
диктовку в черновик; прежний Apple-путь удалён. Полный GUI-191 и весь milestone
не завершены. Контракт и точная граница — [codex-voice.md](codex-voice.md).

## Скрипт и терминал: исходники GUI-187, ещё не установленный выпуск

`.build/project-runs-verified` прошёл 18 Core, 17 Codex, 10 Mac и 6 iPad
проверок без ошибок, пропусков и runtime warnings. После размещения переключателя
терминала рядом с проектом отдельно прошёл `.build/project-runs-window`:
настоящий WebKit/xterm, кириллический ввод, разбор ANSI, сохранённая команда,
обрыв и повторное подключение к тому же запуску. Снимки `project-terminal-ansi`
и узкого чата осмотрены: виден зелёный текст, управление остаётся внутри панели.

`.build/project-runs-live.json` подтверждает настоящий PTY установленного
Codex: русский текст и emoji, изменение размера, ожидание 13 секунд, холодное
чтение того же UUID, нормальный выход и остановка второго процесса. Модель не
вызывалась. Неопределённый ввод и перезапуск Mac отдельно проверены без повторного
исполнения. Контракт — [project-runs.md](project-runs.md). Установленная пара
пока 0.3.25 (28); физический запуск с iPad после выпуска ещё открыт.

Дополнительная проверка голоса `.build/voice-probe/media.log` дошла через
авторизацию ChatGPT до настоящего WebRTC `connected`, открытого канала событий
и `session.started`, после чего сеанс остановлен. Микрофон не использовался.
Это ещё не доказательство передачи речи, распознавания и перебивания.

## Обсуждение кода: исходники GUI-188, ещё не установленный выпуск

`.build/code-discussion-verified` прошёл выбранный объединённый маршрут кода,
пометок, файлов и чата: Core, 45 MCP, Mac и 16 iPad/UI-проверок без ошибок,
пропусков и runtime warnings. Подробные количества и состав сохранены в
квитанции этой проверки. Нативное выделение создаёт неизменяемый общий контекст
с настоящим изображением TextKit и чернил. Живой WebKit открыл ссылку в код и
передал сохранение настоящего сообщения; вложенный script удалён санитайзером.
Переход на строку, возврат к рассмотренному фрагменту и открытие старого
материала после замены текста сохраняют присутствие доски. Сохранённый ответ
получил ссылку на разговор, но не чужое нынешнее выделение.

Изображения `code-pinned-native-ink` и `code-frozen-textkit-material` осмотрены
в `.build/code-discussion-native-fifth/attachments`: текст расположен правильно,
штрих действительно входит в PNG. Предварительные проходы выявили ошибки
тестовой заготовки и преобразований типов; после исправлений повторён
объединённый набор. Контракт — [code-discussion.md](code-discussion.md).
Установленная пара остаётся 0.3.25 (28); физический разговор, правка настоящего
файла агентом с iPad и повторная установка этого дополнения ещё открыты.

Проверка голоса `.build/voice-probe` использовала нынешнюю авторизацию ChatGPT
без чтения или выдачи секретов. WebSocket v3 требует отдельный API-ключ и не
подходит. WebRTC принял настоящее предложение WebKit, вернул started и sdp,
после чего сеанс явно остановлен. Микрофон не включался; это доступность
согласования, не проверка распознавания или двустороннего разговора.
