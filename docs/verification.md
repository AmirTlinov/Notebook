...
Срез: **26 сентября 2026**, последний зафиксированный и отправленный commit —
`4483fefa82ab7fbc65d69401b713d60e238cc904`. Это сводка состояния, не журнал запусков.
...
Эти прежние результаты не являются новым полным прогоном на `4483fefa`.

## Незакрытые условия

| Условие | Фактическое состояние; что требуется для закрытия |
|---|---|
| **Raw-page Redo: правильный конечный кадр** | Последняя чистая Release-попытка `b3ba3811` прошла два полных journey и остановилась на третьем: Undo/Redo оба показывают 96 тёмных пикселей. Новый обычный Release-повтор после всех изменений ещё нужен. [Отказ](audit-evidence/2026-09-25/release-b3ba3811-ten-journey-failure.json). |
