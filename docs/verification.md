...
- [GUI-295](https://linear.app/main-cluster/issue/GUI-295): **не закрыта**.
  Отдельные пути перелистывания и чернил исправлены, но raw-page Redo, перенос/отмена
  смешанного выбора и строгие физические условия ниже остаются открытыми.
...
| **Raw-page inverse ≤100 мс** | На physical137 baseline первый Redo — **143,981 мс**, correct=true; ещё три pen-stage превышают 100 мс. Candidate прошёл эти четыре inverse-цикла, но ухудшил все strict pen/eraser samples и удалён целиком — timing-fix не заявлен. Ранее same-command-buffer readback подтвердил правильный final drawable при старом окне; нынешний phase-run имеет post-GPU CA commit ещё за **162,111 мс до failed capture**. Ни GPU, ни CA callback не доказывают своевременный показ. [137](audit-evidence/2026-09-25/ui-phase-transaction-rejected-137.json), [drawable](audit-evidence/2026-09-25/final-drawable-redo-probe.json), [фазы](audit-evidence/2026-09-25/raw-redo-phase-identity-video.json). |
| **U10: первый перенос, атомарный показ и финальная пара** | **139: полный cold-маршрут FAIL, 3/3 адресные регрессии PASS.** Владение отменяемой позой и единственная подготовка шейдеров проверены, но этого недостаточно для атомарного показа: второй перенос и отмена снова дали по одному смешанному кадру. Первый drag уже correct за 109,998 мс (>100), lift correct за 19,245 мс; поздние целые позы второго drag/cancel за 72,942/76,631 мс не отменяют FAIL. Предыдущий138 показал 0 смешанных кадров только в своём запуске; он не закрывает нынешний отказ. [139](audit-evidence/2026-09-25/user-u10-selection-preparation-139.json), [138](audit-evidence/2026-09-25/cancel-retirement-and-cold-phases-138.json), [137](audit-evidence/2026-09-25/ui-phase-transaction-rejected-137.json). |
...
| **Физический ink response ≤20 мс** | Physical137 baseline: warm pen/eraser по одному PASS, OS max **18,536/19,599 мс**, но cold first dot **41,788 мс — FAIL**. Это не закрывает прежние отказы. Единственный UI-phase clock candidate отвергнут: все **120/120** обязательных OS samples каждого replay >20 мс (max **44,756/47,646 мс**); cold **34,643 мс** тоже FAIL. Missing samples нет; записанные thermal nominal/lowPower=false. Кандидат полностью удалён, новый display clock не оставлен. [137](audit-evidence/2026-09-25/ui-phase-transaction-rejected-137.json), [прежний отрицательный A/B](audit-evidence/2026-09-25/warm-transaction-rejected.json). |
