...
**Файловый доступ ещё не принят:** `open` папки проекта продолжает ждать ответа
системы. Журнал macOS фиксирует отказы защиты файлов для Notebook и его дочерних
процессов; точный вид системного запроса на экране не подтверждён. Нужна проверка
разрешения Notebook на Mac, не обход через процесс с чужими правами. Исправлена
блокировка сохранения, а не объявлен успешный доступ к проекту. Ожидавшее на iPad
