...
## 17 сентября — QuickShape не ждёт подъёма ладони (GUI-205)
...
подъёма всей руки фигура появляется сразу. Причина не в классификаторе и не
в его hold-таймере: `NotebookContactObserver` продолжал держать input barrier
за scene-палец, чей жест уже отменён началом Pencil.
...
Амиру предложен тот же Pencil-жест с оставленной на экране ладонью; ответа
после установки пока нет. GUI-205 остаётся In Progress, полная парная приёмка
не заявлена.
