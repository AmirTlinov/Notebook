  ...
  enum MacDocumentLaunchFixture {
    ...
    /// The smoke route waits for the actual publisher's pixels and the working window,
    ...
    static func writeProof(model: NotebookAppModel) async {
      ...
      while ContinuousClock.now < deadline {
        ...
        if let bytes = try? Data(contentsOf: model.store.currentViewRevisionURL),
          ...
          NSApplication.shared.activationPolicy() == .regular, !workWindows.isEmpty {
          proof = ["status": "ready", "workingWindows": workWindows.count, "surface": "document",
      ...
      do {
        if proof["status"] as? String == "ready" {
          // Keep the pixels alongside the receipt so the launch proof can be
