...
@MainActor enum NotebookTextObjectClipboard {
  ...
  static func paste(nextTo target: NotebookNativeTextTarget, model: NotebookAppModel) {
    guard let text = UIPasteboard.general.string, !text.isEmpty, text.utf8.count <= 65_536 else { return }
    ...
    let textSize = NotebookTextTypography.attributed(text,style:style).boundingRect(
      with:.init(width:width,height:100_000),options:[.usesLineFragmentOrigin,.usesFontLeading],context:nil).size
    let size = SpatialPoint(x:width,y:max(1,ceil(textSize.height)))
    ...
    Task { _ = await model.insertClipboardFragment(.init(elements:[element],size:size),at:destination) }
  }
}
