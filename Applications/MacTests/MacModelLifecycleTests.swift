...
final class MacModelLifecycleTests: XCTestCase {
  ...
  func testPersonCanMoveAndRemoveIPCAuthoredElements() async throws {
    ...
    let movedFrame = model.elementPresentationFrame(pageElement, fallback: moved.frame)
    XCTAssertEqual(movedFrame.x, page.size.width - 240)
    XCTAssertEqual(movedFrame.y, 0)
    ...
    let resizedFrame = model.elementPresentationFrame(pageElement, fallback: resized.frame)
    XCTAssertEqual(resizedFrame, PageRect(x: page.size.width - 240, y: 0, width: 200, height: 260),
      "A second accepted contact projects from the first draft before the writer publishes")
    let saved = await model.finishPendingPersistence()
    XCTAssertTrue(saved)
    let published = try XCTUnwrap(model.pages[page.id]?.element(id: moved.id))
    let durable = try XCTUnwrap(try store.loadPage(page.id).element(id: moved.id))
    XCTAssertEqual(published.frame, resizedFrame)
    XCTAssertEqual(durable.frame, resizedFrame)
    XCTAssertEqual(published.source, moved.source)
    XCTAssertEqual(published.state, moved.state)
    XCTAssertEqual(durable.source, moved.source)
    XCTAssertEqual(durable.state, moved.state)
    ...
    let coverFrame = model.elementPresentationFrame(coverElement, fallback: .init(x: movedCover.frame.x,
      y: movedCover.frame.y, width: movedCover.frame.width, height: movedCover.frame.height))
    XCTAssertEqual(coverFrame, PageRect(x: 0, y: WorkspaceItemGeometry.notebook.height - 190, width: 260, height: 190),
      "The accepted cover pose is visible before canonical publication")
    let coverSaved = await model.finishPendingPersistence()
    XCTAssertTrue(coverSaved)
    let publishedCover = try XCTUnwrap(model.board?.element(id: movedCover.id))
    let durableCover = try XCTUnwrap(try store.readSpatialElement(boardID: boardID, elementID: movedCover.id))
    let expectedCover = SpatialRect(x: coverFrame.x, y: coverFrame.y, width: coverFrame.width, height: coverFrame.height)
    XCTAssertEqual(publishedCover.frame, expectedCover)
    XCTAssertEqual(durableCover.frame, expectedCover)
    model.selectElement(pageElement); model.deleteElement(pageElement)
    model.selectElement(coverElement); model.deleteElement(coverElement)
