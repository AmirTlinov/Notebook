...
@MainActor final class NotebookGraphicSelectionUITests: XCTestCase {
  ...
  private func scenario(onPage: Bool) {
    ...
    b.tap()
    XCTAssertTrue(app.buttons["delete-graphic-selection"].waitForExistence(timeout:3))
    ...
    link.tap()
    app.buttons["element-actions-menu"].tap(); app.buttons["Не добавлять касанием"].tap()
