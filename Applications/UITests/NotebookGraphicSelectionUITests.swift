...
@MainActor final class NotebookGraphicSelectionUITests: XCTestCase {
  ...
  func testMoveWholeWithPassiveMembersAndRestore() {
    ...
    notebookContextAction("Выбрать группу",on:node,in:app)
  ...
  private func scenario(onPage: Bool) {
    ...
    notebookContextAction("Выровнять",on:a,in:app); notebookMenuAction("По левому краю",in:app)
    ...
    XCTAssertEqual(neighbour.staticTexts.matching(NSPredicate(format:"label BEGINSWITH 'Runtime '")).firstMatch.label,runtime)
    XCTAssertTrue(neighbour.staticTexts["Count 1"].exists)
    notebookContextAction("Дублировать",on:a,in:app)
    ...
    notebookContextAction("Удалить выбранные фигуры",on:app.images.matching(identifier:"Узел +").allElementsBoundByIndex.first(where:{$0.isSelected}) ?? a,in:app)
