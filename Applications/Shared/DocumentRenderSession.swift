...
final class DocumentSourceSnapshot {
  ...
  func acceptLayout(_ receipt: NSDictionary, geometry: WorkspaceItemGeometry) throws -> DocumentLayoutRecord {
    let measured = try DocumentLayoutRecord(receipt: receipt, source: message, geometry: geometry)
    if let layout {
      guard layout.matches(measured) else { throw DocumentSessionError.inconsistentLayout }
      return layout
    }
    layout = measured
    return measured
...
final class DocumentLayoutRecord {
  ...
  init(receipt: NSDictionary, source: DocumentSourceMessage, geometry: WorkspaceItemGeometry) throws {
    guard receipt["sourceKey"] as? String == source.key,
    ...
    let blockIDs = Set(source.blocks.map(\.id))
    ...
    self.pageCount = count; self.regions = regions
  ...
  func matches(_ other: DocumentLayoutRecord) -> Bool {
    guard pageCount == other.pageCount, width == other.width, height == other.height,
      regions.count == other.regions.count else { return false }
