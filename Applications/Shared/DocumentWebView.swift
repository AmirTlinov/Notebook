...
final class DocumentSnapshotCache {
  ...
  func image(
    ...
    return entry?.token == Self.token(document: document, state: state)
      ? entry?.image
      : nil
  }
  ...
  nonisolated static func token(
    ...
    return "\(document.contentStamp.counter)@\(document.contentStamp.actor.uuidString.lowercased())|\(state.stamp.counter)@\(state.stamp.actor.uuidString.lowercased())"
  }
}
