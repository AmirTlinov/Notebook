...
final class SceneCameraSettlement {
  ...
  func start(from: SessionPresence, to: SessionPresence, duration: Double, bounce: Double,
    publish: @escaping (SessionPresence, Bool) -> Void, completion: @escaping () -> Void) {
    cancel()
    guard from.boardID == to.boardID, duration > 0 else { publish(to, true); completion(); return }
    ...
    #endif
    guard let link else { cancel(); publish(to, true); completion(); return }
    link.preferredFrameRateRange = .init(minimum: 30, maximum: 120, preferred: 120)
    link.add(to: .main, forMode: .common)
    publish(Self.sample(from: from, to: to, fraction: 0), false)
  }
  ...
  private func advance(at time: Double) {
    ...
    if elapsed >= duration {
      ...
      publish(Self.sample(from: from, to: to, fraction: spring.value(target: 1.0, time: elapsed)), false)
    }
  }
  ...
  static func sample(from: SessionPresence, to: SessionPresence, fraction: Double) -> SessionPresence {
    ...
    let amount = fraction
    let delta = from.camera.center.delta(to: to.camera.center)
    let camera = SpatialCamera(center: from.camera.center.offsetBy(x: delta.x * amount, y: delta.y * amount),
      scale: exp(log(from.camera.scale) + log(to.camera.scale / from.camera.scale) * amount))
