...
struct NotebookPagePreparation: Sendable {
  ...
  static func read(store: NotebookStore, itemID: UUID, index: Int, root: String,
    ...
    return try store.readTransaction { store in
      ...
      let undo = try store.nativeHistory(domain: .page(id), actor: actor)
      let redo = try store.nativeRedoHistory(domain: .page(id), actor: actor)
      return Self(position: position, page: page, revision: revision, projection: projection,
        undo: undo, redo: redo)
    }
  }
