...
@MainActor final class NotebookRunControllerTests: XCTestCase {
  ...
  func testRealTerminalParsesPTYAndReconnectsSameProcessWithoutMovingPaper() async throws {
    ...
    chat.files.showTerminal(true); chat.files.resizeTerminal(fraction: 0.37)
    chat.files.showTerminal(false); chat.files.showTerminal(false)
    XCTAssertEqual(chat.files.window.terminal, false, "A repeated collapse is not a toggle back to open")
    XCTAssertEqual(chat.files.window.terminalFraction, 0.37)
    let hiddenSaved = await queue.flush(); XCTAssertTrue(hiddenSaved)
    XCTAssertEqual(try store.fileWindow(author: author, computer: computer).terminal, false)
    XCTAssertEqual(chat.runs.record?.id, id); XCTAssertEqual(starts, 1)
    chat.files.showTerminal(true)
