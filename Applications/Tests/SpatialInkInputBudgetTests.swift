...
final class SpatialInkInputBudgetTests: XCTestCase {
  ...
  func testInstalledBoardGrowsToTheNewViewportBeforeAcceptingInkOutsideItsOldBacking() async throws {
    ...
    let expectedSize = BoardPortalProjection.renderViewport(viewport: viewport)
    XCTAssertEqual(canvas.bounds.width, max(viewport.x, expectedSize.x), accuracy: 0.001)
    XCTAssertEqual(canvas.bounds.height, max(viewport.y, expectedSize.y), accuracy: 0.001)
