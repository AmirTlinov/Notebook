...
@MainActor final class NotebookPageLifecycleUXTests: XCTestCase {
  ...
  private func checkImmediateAcceptedCurlSource(erasingShape: Bool) async throws {
    ...
    let page = try XCTUnwrap(model.activePage), beforeStamp = model.activePage!.drawingStamp
    ...
    curl.onFrameResolved = { image, progress, receipt in
      if captured == nil, receipt.completion.permitsProgress {
        ...
        phases.append("captured source: progress=\(progress) receipt=\(receipt) sourceStable=\(ink.isStableFramePresented) sourceStamp=\(String(describing: model.pages[page.id]?.drawingStamp))")
      }
      receiveFrame?(image, progress, receipt)
    }
    defer {
      curl.onFrameResolved = receiveFrame
    ...
    XCTAssertEqual(capturedReadiness?.completion.permitsProgress, true)
