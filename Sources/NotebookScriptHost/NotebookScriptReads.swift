...
enum NotebookScriptAPI {
  ...
  static var help: JSONValue { .object([
    ...
    "exports": .string("export admits one immutable saved source/state cut from a WAL snapshot; keep jobID and read exportStatus after JS ends. At most two jobs render in the same pinned offline canonical print owner as paper, without touching a live program. Source/state changes before publication fail revision_conflict. The receipt binds cutSHA256/stateRevision and all artifacts. Current inline limits: TeX <=4 MiB, PDF <=16 MiB, prepared images <=8 MiB, PDF plus images <=17 MiB."),
