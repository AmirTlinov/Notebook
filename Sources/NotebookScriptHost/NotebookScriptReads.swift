...
enum NotebookScriptAPI {
  ...
  static var help: JSONValue { .object([
    ...
    "effects": .array(["transaction(key, action)", "undo(key, {actionID})", "point(key, {references, contextID?, replyTo?})",
      "present(key, {view, steps})", "cancelPresentation(key, {id})", "export(key, {documentID, format?:pdf|png, pageIndex?, pixelWidth?})", "cancelExport(key, {jobID})"].map(JSONValue.string)),
