...
extension NotebookStore {
  ...
  func validatedManifest(_ change: NotebookDurableChange, partHash: String? = nil, historical: Bool = false) throws -> NotebookChangeManifest {
    ...
    guard (manifest.format == 4 || manifest.format == 5 || manifest.format == 6 || manifest.format == 7 || manifest.format == 8 || manifest.format == 9 || manifest.format == NotebookChangeManifest.currentFormat) || (historical && manifest.format == 3) else {
