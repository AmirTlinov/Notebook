...
public struct NotebookGraphic: Codable, Equatable, Sendable {
  public enum Shape: String, Codable, Sendable { case ellipse, rectangle, triangle, diamond, plus, connector, freehand }
  ...
  public init(shape: Shape = .ellipse, style: Style = .init(), label: String = "",
    ...
    connection: NotebookGraphicConnection? = nil, vertices: [SpatialPoint]? = nil, cornerRadius: Double? = nil, freehand: NotebookFreehand? = nil, transform: NotebookGraphicTransform? = nil) {
    ...
    self.connection = connection; self.vertices = vertices; self.cornerRadius = cornerRadius; self.freehand = freehand; self.transform = transform
  }
  ...
  public func applying(_ patch: JSONValue) throws -> Self {
    ...
    for (part, supplied) in patch.object {
      ...
      } else { value = value.setting(part, ["vertices", "cornerRadius", "freehand", "transform"].contains(part) && supplied == .null ? nil : supplied) }
    }
  ...
  static let causalFields = ["shape", "style", "label", "representation", "visible", "sourceInkIDs", "vertices", "cornerRadius", "freehand", "transform"]
  ...
  var causalPaths: [[String]] {
    Self.causalFields.filter { ($0 != "vertices" || vertices != nil) && ($0 != "cornerRadius" || cornerRadius != nil) && ($0 != "freehand" || freehand != nil) && ($0 != "transform" || transform != nil) }.map { [$0] } + (connection == nil ? [] : NotebookGraphicConnection.causalFields.filter { ($0 != "bendPosition" || connection?.bendPosition != nil) && ($0 != "routing" || connection?.routing != nil) }.map { ["connection", $0] })
  }
...
public enum NotebookGraphicGeometry {
  ...
  public static func polygon(_ graphic: NotebookGraphic) -> [SpatialPoint]? {
    ...
    case .ellipse, .plus, .connector, .freehand: return nil
    }
  }
  public static func hitTest(_ graphic: NotebookGraphic, width: Double, height: Double,
    ...
    if graphic.transform != nil || graphic.shape == .freehand {
  ...
  public static func outlineDistance(_ graphic: NotebookGraphic, width: Double, height: Double,
    ...
    case .connector, .freehand: return .infinity
    }
  }
}
