...
const exportJob=object({status:z.enum(["missing","queued","running","saved","failed","interrupted"]),jobID:id.optional(),documentID:id.optional(),contentRevision:text.optional(),stateRevision:text.optional(),cutSHA256:text.optional(),moment:z.literal("saved").optional(),receipt:object({cutSHA256:text,stateRevision:text,cut:printMapArtifact,documentID:id,pdfPath:text,texPath:text,pdfSHA256:text,byteCount:number,log:text,packageSHA256:text.optional(),assets:z.array(object({path:text,sha256:text})).optional(),sourceMap:printMapArtifact.optional(),syncTeX:printMapArtifact.optional()}).optional(),error:json.optional()});
