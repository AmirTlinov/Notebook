...
const catalogue: Record<string,{input:z.ZodType;returns:string;example:string;old?:string[]}> = {
  ...
  observe:{input:z.object({contextID:id.optional(),target:z.object({kind:z.enum(["page","document"]),id}).strict().optional(),elementID:text.optional(),blockID:text.optional(),limit:z.number().int().min(1).max(32).optional(),includeImage:z.boolean().optional(),since:z.record(text,z.json()).optional()}).strict(),returns:"{header,presence,content,contexts,connection,cursor,changeKeys,changes,visual:{status,receipt?,artifact?}}; pass previous changeKeys as since, which works across clients without sidecar memory. Address a page element with target+elementID, a document block with target+blockID. Preview reads at most limit (default 32) member records. Unchanged content returns unchanged:true without decoding bodies; changing scope requires a new bounded read. Images require includeImage:true. Source freshness is independent from connection and iPad display.",example:"await emit(await nb.observe());",old:["notebook_observe"]},
