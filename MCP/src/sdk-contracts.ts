...
const catalogue: Record<string,{input:z.ZodType;returns:string;example:string;old?:string[]}> = {
  help:{input:z.object({topic:text.optional()}).strict(),returns:"Index, or exact method input schema, result description and example. operations is a compact index of 19 variants; operation/<name> gives one exact schema. interactive explains embedded notebook.ready(promise); execution explains terminal status and output pagination.",example:"await emit(await nb.help('operation/createDocument'));"},
  observe:{input:z.object({contextID:id.optional(),since:z.record(text,z.json()).optional()}).strict(),returns:"{header,presence,content,contexts,connection,cursor,changeKeys,changes,visual:{status,receipt?,artifact?}}; pass previous changeKeys as since, which works across clients without sidecar memory. Source freshness is independent from connection and iPad display.",example:"await emit(await nb.observe());",old:["notebook_observe"]},
  ...
  page:{input:z.object({id:id.optional()}).strict(),returns:"{page:{id,size,elements,...},agentRevision,drawingRevision,cursor}; no implicit camera movement.",example:"const p=await nb.page({id:args.pageID}); await emit(p);",old:["notebook_read_page"]},
  ...
  search:{input:z.object({query:text.min(1).max(500),limit:z.number().int().min(1).max(100).optional()}).strict(),returns:"Native search results with physical paths and exact references; no handwriting OCR is inferred.",example:"await emit(await nb.search({query:'Transformer',limit:10}));",old:["notebook_search"]},
...
for (;;) {
  ...
  examples:{
    ...
    graphic:"insertElement: {kind:'graphic',source:'',frame:{x:20,y:20,width:100,height:100},graphic:{shape:'ellipse',style:{stroke:{red:0,green:0,blue:0},strokeWidth:2},label:'+',representation:'geometry',visible:true,sourceInkIDs:[]}}. Native shapes: ellipse, rectangle, triangle, diamond, plus, connector. Triangle/diamond may carry vertices: 3/4 convex ordered points {x,y} normalized to [0,1] in frame; omit or set null for the canonical outline. For a board add worldOrigin. updateElement accepts a graphic patch, e.g. {graphic:{label:'?'}}; sourceInkIDs cannot be patched. convertInkToElement uses the same payload with existing sourceInkIDs and requires inkRevision.",
