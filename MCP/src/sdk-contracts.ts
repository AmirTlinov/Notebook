...
const query=z.object({kind:z.enum(["observation","workspaceHeader","itemHeaders","itemHeader","itemLifecycle","workingSet","sceneWindow","scenePaintOrder","page","pageHeader","pageElement","documentHeader","document","documentState","documentBlock","boardItem","boardElement","boardContentRevision","ownerBoard","notebookPages","notebookDirectory","notebookPosition","spatialInk","presence","selection","attentionEvidence","contexts","contextEntries","actions","currentViewReceipt","pageVisionReceipt","targetRenderReceipt","renderRequests","delivery","actionSnapshots","runtime","codeFragment","codeFragments"]),
...
const catalogue: Record<string,{input:z.ZodType;returns:string;example:string;old?:string[]}> = {
  help:{input:z.object({topic:text.optional()}).strict(),returns:"Index, or exact method input schema, result description and example. operations is a compact index of 22 variants; operation/<name> gives one exact schema. interactive explains embedded notebook.ready(promise); execution explains terminal status and output pagination.",example:"await emit(await nb.help('operation/createDocument'));"},
  observe:{input:z.object({contextID:id.optional(),target:targetSchema.optional(),elementID:text.optional(),blockID:text.optional(),ids:observationScope.shape.ids,fields:observationScope.shape.fields,expand:observationScope.shape.expand,bounds:sceneBoundsSchema.optional(),limit:z.number().int().min(1).max(32).optional(),includeImage:z.boolean().optional(),since:text.max(196608).optional(),next:text.max(196608).optional()}).strict(),returns:"Snapshot of a bounded observation: data.objects, data.header, data.checkpoint, data.mode. coverage.next drains the same frozen scope; pass data.checkpoint as since only after complete. Fields/one-hop expansion/geometry remain Core-owned. Without target/contextID, data.selection reports the authenticated active UI choice (known or unknown); a selected element is read by exact physical target/id, even beyond the preview window. Selection grants no write scope. contextID reads only that historical source, never today’s camera/basis. includeImage:true returns current-view pixels only for an active known scene with a matching physical surface (otherwise current_scene_unknown/pending); for historical sources use attention({contextID,referenceID}), and for explicit targets use render. Missing source pixels are never replaced with the current view.",example:"let r=await nb.observe({target:args.target}); while(r.coverage.next) r=await nb.observe({target:args.target,next:r.coverage.next}); await emit(r);",old:["notebook_observe"]},
  read:{input:query,returns:"itemLifecycle with an item id returns its complete stored extent without loading page/document bodies, plus frozen workspace, actual parent and lifecycle basis. bodyRecordCount counts page/document/state records only, not a total deletion cost or mutation authority. Unseen source, ink, state, membership or child-board causal changes invalidate that extent; ordinary itemHeader does not provide it. selection returns known/unknown session identity, generation, physical surface and typed choice from the existing UI owner; its basis has no owners and grants no authority. Snapshot<T> selected by query.kind. data is one object or collection, never values[0]. basis contains exact owner versions from the SAME native snapshot, not mutation permissions. coverage.complete is explicit; next can be passed with the same kind to read. Stale continuation fails, without a hidden full rescan.",example:"const s=await nb.read({kind:'pageElement',id:args.pageID,elementID:args.elementID}); await emit(s.data);"},
  ...
  transaction:{input:z.object({key:text.min(1).max(120),action}).strict(),returns:"ActionResult {actionID,actionVersion,changed,basis,publication,next?}. base is a read basis or explicit array of bases; no expected array. Versions never refresh before a write. Missing components -> basis_incomplete, incompatible versions -> basis_conflict, wrong workspace -> basis_workspace_mismatch. Scope remains separate. Canonical small field values and exact after digests are frozen in the native commit with effect.value. Lifecycle changed events are appendPage {target,pageID,item} with the saved header or deletedItem {target,item} with the pre-action header; no page bodies or inverse records. Large changed sets use action({actionID,actionVersion,next}). Retry never substitutes latest state or later undo.",example:"const s=await nb.page({id:args.pageID,elementID:args.elementID}); await emit(await nb.transaction('label',{base:s.basis,summary:'Подпись',operations:[{kind:'updateElement',target:{kind:'page',id:args.pageID},id:args.elementID,values:{graphic:{label:'Обратная связь'}}}]}));",old:["notebook_apply"]},
  undo:{input:z.object({key:text.min(1).max(120),actionID:id}).strict(),returns:"ActionResult for the undo's own immutable version. Later human changes remain preserved. Lifecycle changed events report only actual restoreItem {target,item} or removePage {target,pageID,item?}; a preserved item/page emits no lifecycle event, and original operations are never replayed as results. preservedCount includes retained lifecycle owners; the exact-version undo detail section names them as {target,reason:'lifecycle_owner_continued'}, not invented field changes. Original ActionResult remains unchanged; detailed preserved addresses require actionID and this actionVersion.",example:"await emit(await nb.undo('undo-label',{actionID:args.actionID}));",old:["notebook_undo"]},
...
const lifecycleOwner="Read nb.read({kind:'itemLifecycle',id:itemID}) explicitly. Use its data.target unchanged: a cover with the actual containing boardID, not the root board or a page. Pass the returned basis and declare the exact cover in additionalOwners unless the source context already covers it. This read grants no mutation scope; stale or missing lifecycle evidence is never refreshed during a write.";
const lifecycleExample=(kind:"appendPage"|"deleteItem")=>`const extent = await nb.read({kind:'itemLifecycle',id:args.itemID});
if (!extent.data) throw new Error('Item is missing');
await emit(await nb.transaction('${kind==="appendPage"?"append-page":"delete-item"}', {
  summary:'${kind==="appendPage"?"Добавить лист":"Удалить предмет"}', base:extent.basis, additionalOwners:[extent.data.target],
  operations:[{kind:'${kind}',target:extent.data.target,values:{}}]
}));`;
...
const operationDetails=Object.fromEntries(operationSchema.options.map(schema=>{
  ...
  return [name,{name,description:operationDescriptions[name],input:z.toJSONSchema(schema,{reused:"ref"}),
    ...
    ...(name==="appendPage"||name==="deleteItem"?{owner:lifecycleOwner,example:lifecycleExample(name)}:{}),
