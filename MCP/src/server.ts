...
export const executionOutput = z.union([z.object({
  ...
  fingerprint:z.string(),api_version:z.literal(1),
...
const cursorFields={after_seq:z.number().int().nonnegative().default(0),wait_ms:z.number().int().min(0).max(4000).default(0)};
export const executionInput=z.discriminatedUnion("op",[
  z.object({op:z.literal("start"),run_id:z.uuid(),api_version:z.literal(1),code:z.string().max(262144),args:z.json().optional(),...cursorFields}).strict(),
...
export function createServer(socketPath=defaultSocketPath()):McpServer {
  ...
  server.registerTool("notebook_context",{
    ...
    const data=await runBridge<Value>(socketPath,{command:"scriptContext",scriptContext:{method,arguments:args}},{deadline});
    const descriptors=[data.artifact,...(Array.isArray(data.artifacts)?data.artifacts:[]),
      ...(method==="observe"&&data.visual&&typeof data.visual==="object"?[(data.visual as Value).artifact]:[])].filter(Boolean).slice(0,4);
  ...
  server.registerTool("notebook_execute",{
    ...
    return response(async(deadline)=>{
      ...
      const request:Value={op:input.op,runID:input.run_id,afterSequence:input.after_seq,waitMilliseconds};
      if(input.op==="start") {
