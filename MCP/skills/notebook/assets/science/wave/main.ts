...
type State={draft:Parameters;accepted:Parameters|null;tab:'model'|'recording';playhead:number;rate:number};
function stateFrom(value:unknown):State {
 const v=value as Partial<State>|null;
 return {draft:parameters(v?.draft),accepted:v?.accepted?parameters(v.accepted):null,tab:v?.tab==='recording'?'recording':'model',playhead:number(v?.playhead,0,0,provenance.duration),rate:[.5,1,1.5,2].includes(v?.rate??0)?v!.rate!:1};
}
