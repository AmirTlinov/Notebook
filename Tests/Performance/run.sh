...
xcrun -sdk macosx metal -c "$ROOT/Applications/Shared/CompactInk.metal" -o "$BUILD/compact.air"
xcrun -sdk macosx metallib "$BUILD/ink.air" "$BUILD/compact.air" -o "$BUILD/default.metallib"
...
xcrun swiftc -Onone -I "$BUILD" -L "$BUILD" -lNotebookCore -Xlinker -rpath -Xlinker "$BUILD" \
  ...
  "$ROOT/Applications/Shared/InkConnectivity.swift" "$ROOT/Applications/Shared/InkRenderUniforms.swift" \
