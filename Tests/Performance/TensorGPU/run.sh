...
xcrun -sdk macosx metallib "$OUT/reference.air" "$OUT/baseline.air" "$OUT/direct.air" -o "$OUT/ink.metallib"
xcrun swiftc -O -whole-module-optimization "$ROOT/Sources/NotebookCore/InkStrokeGeometry.swift" \
  "$ROOT/Tests/Performance/TensorGPU/Support.swift" "$ROOT/Tests/Performance/TensorGPU/main.swift" -o "$OUT/probe"
{
  ...
  shasum -a 256 "$ROOT/Tests/Performance/TriangleReference.metal" "$ROOT/Sources/NotebookCore/InkStrokeGeometry.swift" "$ROOT/Applications/Shared/InkShaders.metal" \
    "$ROOT/Tests/Performance/TensorGPU/"{TensorInk.metal,Support.swift,main.swift,run.sh} \
