...
func creationUndoMixedDocumentsPreservesOnlyAdoptedOwner() throws {
  ...
  #expect(try f.store.readItemHeader(first) != nil)
  #expect(try f.store.ownerBoardID(of: first) == f.boardID)
  #expect(try f.store.readItemHeader(second) == nil)
...
func creationUndoNestedAdoptionKeepsOnlyConnectedDependencies() throws {
  ...
  #expect(try f.store.readItemHeader(outside) == nil)
...
func creationUndoAcceptedTrailingPageProtectsNotebook() throws {
  ...
  _ = try f.store.undoCollaborationAction(action.id, actor: f.human)
  #expect(try f.store.readNotebookPageDirectory(itemID: first, limit: 4).pages.map(\.position.pageID) == [firstPage, landing.pageID])
...
func creationUndoCausalPlacementAdoptionKeepsPaper() throws {
  ...
  let moved = try moveTestItem(store: f.store, itemID: first, in: f.boardID, to: .init(x: 2000, y: 1000), actor: f.human)
  #expect(moved)
  let returned = try moveTestItem(store: f.store, itemID: first, in: f.boardID, to: center, actor: f.human)
  #expect(returned)
