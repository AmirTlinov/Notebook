...
func pencilUndoKeepsAgentStroke() throws {
  ...
  let human = PageInkAction(tool: .pen, samples: [sample]), agent = PageInkAction(tool: .pen, samples: [sample])
  let pageID = UUID()
  let one = try PageInkDrawing().appending(human).dataRepresentation()
  let two = try PageInkDrawing.decode(one).appending(agent).dataRepresentation()
  var history = PencilUndoHistory()
  history.recordAction(pageID: pageID, before: Data(), after: one)
  let removed = history.removeLastChange(for: pageID, from: two)
  let result = try #require(removed)
  #expect(try PageInkDrawing.decode(result).activeActions.map(\.id) == [agent.id])
}
