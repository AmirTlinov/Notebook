...
struct BoardPlacementMigrationTests {
  ...
  @Test func normalOpenConvertsOnceAndRetainsContentsCursorsAndImmutableBytes() throws {
    ...
    #expect(try databaseVersion(reopened) == NotebookStore.currentDatabaseVersion)
  ...
  func anInterruptedMigrationLeavesTheOriginalRowsAndDeliveryCutIntact(fault: NotebookStorageFault) throws {
    ...
    #expect(try databaseVersion(store) == NotebookStore.currentDatabaseVersion)
  ...
  @Test func anUnknownDatabaseVersionIsNotDowngradedOrWritten() throws {
    ...
    _ = try store.initializeWorkspace(actor: UUID(), pageSize: .init(width: 834, height: 1194))
    let database = try NotebookSQLConnection(url: store.databaseURL, writable: true)
    let unsupported = NotebookStore.currentDatabaseVersion + 1
    try database.run("PRAGMA user_version=\(unsupported)")
    ...
    #expect(try databaseVersion(store) == unsupported)
  ...
  @Test func aNewReplicaUsesACurrentSeedAndThenContinuesTheSameJournal() throws {
    ...
    #expect(manifest.format == NotebookChangeManifest.currentFormat)
