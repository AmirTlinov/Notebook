...
struct NotebookNativeHistoryTests {
  ...
  @Test func mixedOrderAndItsInversesSurviveReopeningWithoutPrioritizingCommands() throws {
    ...
    let undo = try page.prepareInkChange(.remove([last]), stamp: page.drawingStamp.advanced(by: f.actor)!)
  ...
  @Test func aPeerInverseCannotRemainTheHeadOfLocalUndo() throws {
    ...
    let change = try page.prepareInkChange(.remove([last]), stamp: page.drawingStamp.advanced(by: peer)!)
