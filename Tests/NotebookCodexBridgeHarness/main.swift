...
@main struct Proof {
  ...
  static func run() async throws {
    ...
    if args.count == 3, args[1] == "dictation" {
      ...
      let start = Date()
      do {
        let text = try await metadata.transcribeDictation(audio)
        try write(["text": text, "elapsedSeconds": String(Date().timeIntervalSince(start)), "audioBytes": String(audio.count)])
        await metadata.close(); return
      } catch { await metadata.close(); throw error }
    }
